...
template <typename T, typename KeyT = int> class LIRSCache
{
    ...
    BlockStatus get_block_status(KeyT key)
    {
    ...
    bool is_hir(KeyT key) { return get_block_status(key) == BlockStatus::HIR; }
    bool is_lir(KeyT key) { return get_block_status(key) == BlockStatus::LIR; }
    ...
    bool is_full() { return cache_.size() >= capacity_; }
...
public:
    ...
    template <typename F> const bool lookup_update(KeyT key, F slow_get_page)
    {
        auto hit_stack = hash_stack_.find(key);
        auto hit_queue = hash_queue_.find(key);
